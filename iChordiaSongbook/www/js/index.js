...
var iChordia = {
    ...
    buildChords: function() {
        //iMatrix = jQuery('.isong').attr('iInstrument').split('-');
        jQuery('.isong .ichord').each(function(index) {
			var notes = jQuery(this).html();
			//console.log(notes);
			var n=notes.indexOf(" ");
			var el = "div";
			switch(n) {
				case -1:
				  jQuery(this).html('<'+el+' class="cpl cpl1">'+notes+'</'+el+'>');
				  break;
				default:
				  notesArr = notes.split(' ');
				  result = '';
				  for (var note in notesArr) {
					  cssclass = 'cpl cpl' + notesArr.length;
					  if ((note > 0) || ((note < notesArr.length-1))) {
					  	cssclass += ' cpl-left';
					  }
					  if (notesArr[note].length == 0) {
						  notesArr[note] = "&nbsp;";
					  }
					  result += '<'+el+' class="'+ cssclass +'">'+notesArr[note]+'</'+el+'>'
				  }
